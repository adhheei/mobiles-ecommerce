    ...
    products.forEach(product => {
        const productHTML = `
            <div class="col-md-6 col-lg-4 product-item" data-id="${product._id}">
                <div class="wishlist-card ${product.stock === 0 ? 'sold-out' : ''}">
                    <a href="/User/singleProductPage.html?id=${product._id}" class="product-link">
                        <div class="wishlist-img-box">
                            ${product.stock === 0 ? '<span class="sold-out-badge">Out of Stock</span>' : ''}
                            <img src="${product.mainImage}" alt="${product.name}" class="wishlist-img">
                        </div>
                        <h6 class="product-title">${product.name}</h6>
                    </a>
                    <div class="product-price">₹${product.offerPrice}</div>
                    <div class="card-actions">
                        <button class="btn-add-cart" onclick="addToCart('${product._id}')" ${product.stock === 0 ? 'disabled' : ''}>
                            ${product.stock === 0 ? 'Out of Stock' : 'Add to Cart'}
                        </button>
                        <button class="btn-remove" onclick="removeFromWishlist('${product._id}', this)">
