  ...
  try {
    ...
    if (product.stock <= 0 || product.status === "outofstock") {
      return res.status(400).json({
        success: false,
        message:
          "This product is currently out of stock and cannot be added to the wishlist.",
      });
    }
...
const getAllUsers = async (req, res) => {
  try {
    const usersWithOrderCounts = await User.aggregate([
      // 1. Filter out deleted users
      { $match: { isDeleted: { $ne: true } } },

      // 2. Join with the Orders collection
      {
        $lookup: {
          from: "orders", // CRITICAL: Ensure this matches your MongoDB collection name
          localField: "_id", // User's ID
          foreignField: "userId", // The field in Order schema that stores User ID
          as: "userOrders",
        },
      },

      // 3. Create the 'orderCount' field by getting the size of the array
      {
        $addFields: {
      ...
      // 4. Remove sensitive data and the bulky temporary orders array
      {
        $project: {
          password: 0,
          userOrders: 0,
      ...
      // 5. Sort by newest registration first
      { $sort: { createdAt: -1 } },
    ]);
...
const toggleBlockUser = async (req, res) => {
  try {
    ...
    // findByIdAndUpdate is the most direct way to fix a 500 error here
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error during blocking" });
  }
};
